...
@section('breadcrumb', 'Purchase / Pengajuan Pembelian / Edit')

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="mb-4">
        <h2 class="text-lg font-semibold text-gray-900">Edit Pengajuan Pembelian Material</h2>
    ...
    <form action="{{ route('purchase-requests.update', $purchaseRequest) }}" method="POST" id="requestForm">
        @csrf
        @method('PATCH')
        ...
        <x-card class="mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="request_date" value="{{ old('request_date', $purchaseRequest->request_date->format('Y-m-d')) }}" 
                        ...
                        {{ $purchaseRequest->status !== 'pending' ? 'disabled' : 'required' }}>
                    @error('request_date')
                ...
                <div>
                    ...
                    <input type="text" name="letter_number" value="{{ old('letter_number', $purchaseRequest->letter_number) }}" 
                        ...
                        placeholder="Contoh: SPB/001/I/2026" 
                        {{ $purchaseRequest->status !== 'pending' ? 'disabled' : 'required' }}>
                    @error('letter_number')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="letter_date" value="{{ old('letter_date', $purchaseRequest->letter_date->format('Y-m-d')) }}" 
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="purpose" rows="3" 
                        ...
                        placeholder="Jelaskan tujuan/keperluan pembelian material ini" 
                        {{ $purchaseRequest->status !== 'pending' ? 'disabled' : 'required' }}>{{ old('purpose', $purchaseRequest->purpose) }}</textarea>
                    @error('purpose')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-xs">
                    ...
                    <tbody id="materialTableBody" class="divide-y divide-gray-200">
                        @foreach($purchaseRequest->details as $index => $detail)
                            <tr class="hover:bg-gray-50" id="material-row-{{ $index }}">
                                <td class="px-3 py-2">
                                    @if($purchaseRequest->status === 'pending')
                                        <select name="materials[{{ $index }}][material_id]" 
                                            onchange="updateUnit({{ $index }})"
                                            ...
                                            @foreach($materials as $material)
                                                <option value="{{ $material->material_id }}" 
                                                    ...
                                                    {{ $detail->material_id == $material->material_id ? 'selected' : '' }}>
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" id="unit-{{ $index }}" 
                                        value="{{ $detail->material->unit }}" readonly
                                        class="w-20 px-2 py-1 border border-gray-300 rounded text-xs text-center bg-gray-50">
                                </td>
                                <td class="px-3 py-2">
                                    <input type="number" name="materials[{{ $index }}][qty_requested]" 
                                        step="0.01" min="0.01" value="{{ $detail->qty_requested }}"
                                ...
                                <td class="px-3 py-2">
                                    <input type="text" name="materials[{{ $index }}][notes]" 
                                        ...
                                        {{ $purchaseRequest->status !== 'pending' ? 'disabled' : '' }}>
                                </td>
                                @if($purchaseRequest->status === 'pending')
                                    <td class="px-3 py-2 text-center">
                                        <button type="button" onclick="removeMaterial({{ $index }})" class="text-red-600 hover:text-red-800">
        ...
        <div class="flex gap-2">
            ...
            @endif
            <a href="{{ route('purchase-requests.show', $purchaseRequest) }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm rounded-md">
                Kembali
            </a>
        </div>
    </form>
...
@if($purchaseRequest->status === 'pending')
<script>
let materialIndex = {{ $purchaseRequest->details->count() }};
