{{-- resources/views/master/kavling-pembeli/create.blade.php --}}
@extends('layouts.app')

@section('title', 'Tambah Pengaitan Kavling & Pembeli')
@section('breadcrumb', 'Master / Kavling & Pembeli / Create')

@section('content')
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-900">Tambah Pengaitan Kavling & Pembeli</h2>
    ...
    <x-card>
        <form action="{{ route('master.kavling-pembeli.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="location_id" class="w-full px-3 py-2 text-xs border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('location_id') border-red-500 @enderror" required>
                        ...
                        @foreach ($kavlings as $kavling)
                            <option value="{{ $kavling->location_id }}" {{ old('location_id') == $kavling->location_id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_akad" value="{{ old('tanggal_akad') }}" 
                ...
                <div>
                    ...
                    <input type="number" name="harga_jual" value="{{ old('harga_jual') }}" step="0.01" min="0"
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 text-xs border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror" required>
                        ...
                        <option value="akad" {{ old('status') == 'akad' ? 'selected' : '' }}>Akad</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="keterangan" rows="3" 
                        class="w-full px-3 py-2 text-xs border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
