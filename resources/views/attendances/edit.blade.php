...
@section('breadcrumb', 'Payroll / Absensi Tukang / Edit')

@section('content')
<div class="max-w-2xl mx-auto">
    <x-card title="Edit Absensi">
        <form method="POST" action="{{ route('attendances.update', $attendance) }}">
                    ...
                    <select name="worker_id" required class="w-full px-3 py-2 text-sm border rounded @error('worker_id') border-red-500 @enderror">
                        ...
                        @foreach($workers as $worker)
                            <option value="{{ $worker->worker_id }}" 
                                {{ old('worker_id', $attendance->worker_id) == $worker->worker_id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea name="notes" rows="3" class="w-full px-3 py-2 text-sm border rounded @error('notes') border-red-500 @enderror" placeholder="Tambahkan catatan jika diperlukan">{{ old('notes', $attendance->notes) }}</textarea>
                @error('notes')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
