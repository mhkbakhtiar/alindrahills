...
echo "🎨 Creating Complete Views, Layouts & Components..."
...
<head>
    ...
    <title>@yield('title', 'Logistics System')</title>
...
>
    <div class="flex flex-col h-full">
        <!-- Logo -->
        <div class="flex items-center justify-center h-14 border-b border-gray-800 px-4">
        ...
        <nav class="flex-1 overflow-y-auto py-2 text-xs">
            ...
            <div x-data="{ open: {{ request()->is('materials/*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" class="pl-8 space-y-1">
                    ...
                    <a href="{{ route('purchases.index') }}" class="block px-4 py-1.5 hover:bg-gray-800 {{ request()->routeIs('purchases.*') ? 'text-white' : '' }}">Pembelian</a>
            ...
            <div x-data="{ open: {{ request()->is('payroll/*') ? 'true' : 'false' }} }">
                ...
                <div x-show="open" class="pl-8 space-y-1">
                    ...
                    <a href="{{ route('attendance.index') }}" class="block px-4 py-1.5 hover:bg-gray-800">Absensi</a>
            ...
            <div x-data="{ open: false }">
                ...
                <div x-show="open" class="pl-8 space-y-1">
                    <a href="{{ route('reports.stock') }}" class="block px-4 py-1.5 hover:bg-gray-800">Laporan Stok</a>
                    <a href="{{ route('reports.activities') }}" class="block px-4 py-1.5 hover:bg-gray-800">Laporan Kegiatan</a>
                    <a href="{{ route('reports.payroll') }}" class="block px-4 py-1.5 hover:bg-gray-800">Laporan Payroll</a>
                </div>
            </div>
            @endif
        </nav>
...
cat > resources/views/layouts/header.blade.php << 'EOF'
...
cat > app/Http/Middleware/LogActivity.php << 'EOF'
<?php
...
echo "✅ Views, Layouts & Middleware created!"
EOF

chmod +x generate_views_middleware.sh
echo "✅ Script created!"